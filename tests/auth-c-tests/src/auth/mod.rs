use crate::{calculate_ripemd160, calculate_sha256};
...
use std::{process::Stdio, result, vec};
use std::{
...
pub trait Auth: DynClone {
    ...
    // if config.customize_args is true,
    fn custom_args(&self) -> Bytes {
        panic!("unsupport custom_args");
    }
}
...
#[derive(Clone)]
pub struct DogecoinAuth(pub BitcoinAuth);
impl DogecoinAuth {
...
impl Auth for OwnerLockAuth {
    fn get_pub_key_hash(&self) -> Vec<u8> {
        let hash = CellOutput::calc_data_hash(&crate::ALWAYS_SUCCESS);
