...
int main() {
    ...
    for (size_t i = 0; i < auth_num; i++) {
        ...
        if (lock_bytes_seg.size < witness_offset + 4) {
            return CKB_INVALID_DATA;
        }
        uint32_t sign_size = *(uint32_t *)(lock_bytes_seg.ptr + witness_offset);
        if (lock_bytes_seg.size < witness_offset + 4 + sign_size) {
            return CKB_INVALID_DATA;
        }
        uint8_t *signature = lock_bytes_seg.ptr + witness_offset + 4;
